...
def pretune(co,data):
#实现对信号预加重，co为预加重系数，data为加重对象,一维数组,突出高频部分
    size_data = len(data)
...
def writewav(audiodata,samplerate,audiopath):
    ...
    for v in outData:
        ...
        outwave.writeframes(struct.pack('h', np.clip(int(v * 64000 / 2), -32768, 32767))) #改动
...
#音频文件路径，帧长时间单位，帧的重叠率稍微大些
    ...
    for file in os.listdir(directory):
        ...
        if wavfilename.endswith(".wav"):
            ...
            file_path = audiofilespath + '/' + wavfilename  ##音频路径
            ...
            # 去除静音
            ds_data = dsilence(data, 0.2, sr)  # 0.5是无声阈值，越高留下越少 改动
            ...
            winfunc = choose_windows('Hanning', frame_length)
            frames,nf = enframe(ds_data, frame_length, hop_length, winfunc)#(帧数，帧长) #改动 ds_data改为data
            if nf != 1:

...
class RandomDataset(Dataset): #产生测试集/验证集
    def __init__(self, transform=None, dataloader=default_loader):
        ...
        with open(rootPath+'\original/hard_test.txt', 'r') as fid:
...
class BatchDataset(Dataset): #产生训练集
    def __init__(self, transform=None, dataloader=default_loader):
        ...
        with open(rootPath+'\original/hard_train.txt', 'r') as fid:
            self.imglist = fid.readlines()
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, num_workers=0, batch_size=batchsize, pin_memory=True, shuffle=True)

    net = ResNeSt().to(device)
    # test_model = "./models_pkl/checkpoint_hard_samples.pth.tar"
...
def val(val_loader, net, criterion,e,epoch):
    ...
    f1 = open('./test_negative_np_hard_samples_epoch' + str(e) + '.txt', 'w')
    net.eval()
    with torch.no_grad():
...
def polt_curve(epoch,tarin_Accuracy_list,tarin_Loss_list,test_Accuracy_list,test_Loss_list):
    ...
    plt.savefig("./train_test_curve_np_hard_samples_epoch50.jpg")
    plt.show()

...
#         self.dataloader = dataloader
#
#         with open(rootPath+'\original/test2.txt', 'r') as fid:
#             self.imglist = fid.readlines()
#
#     def __getitem__(self, index):
#         image_name, label = self.imglist[index].strip().split(',,,')
...
#         self.dataloader = dataloader
#
#         with open(rootPath+'\original/original/train1.txt', 'r') as fid:
#             self.imglist = fid.readlines()
#
#         self.labels = []
...
def main():
    ...
    result = open('./models_pkl/result_allCamera_focal_50_0.5_1000.txt', 'w')
...
def val(val_loader, net, criterion,e,epoch):
    ...
    f1 = open('./test_negative_allCamera_focal_50_0.5_1000.txt', 'w')
    net.eval()
    with torch.no_grad():
...
def polt_curve(epoch,tarin_Accuracy_list,tarin_Loss_list,test_Accuracy_list,test_Loss_list):
    ...
    plt.savefig("./train_allCamera.jpg")
    plt.show()

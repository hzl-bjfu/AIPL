...
def triplet_loss(alpha=0.2):  # alpha是令N至少比P大多少的超参，alpha越大，理想情况下N比P的大的越多
    def _triplet_loss(y_pred, Batch_size):
        ...
        # anchor, positive, negative,2negative = y_pred[:int(Batch_size)], y_pred[int(Batch_size):int(2 * Batch_size)], y_pred[int(2 * Batch_size):int(3 * Batch_size)], y_pred[int(3 * Batch_size):]
        ...
        # neg_dist2 = torch.sqrt(torch.sum(torch.pow(anchor - negative2,2), axis=-1))  #cfy

        keep_all = (neg_dist - pos_dist < alpha).cpu().numpy().flatten()  # 不满足N-P<alpha都属于错误样本，要计算其loss
        # keep_all = (neg_dist + neg_dist2 - pos_dist < alpha).cpu().numpy().flatten()  #cfy
        hard_triplets = np.where(keep_all == 1)
        pos_dist = pos_dist[hard_triplets].cuda()
        neg_dist = neg_dist[hard_triplets].cuda()
        # neg_dist2 = neg_dist[hard_triplets].cuda()  #cfy

        basic_loss = pos_dist - neg_dist + alpha
        # basic_loss = pos_dist - neg_dist - neg_dist2  + alpha  #cfy
...
def triplet_loss_N2(alpha=0.2,alpha2=0.5):  # alpha是令N至少比P大多少的超参，alpha越大，理想情况下N比P的大的越多
    def _triplet_loss_N2(y_pred, Batch_size):
        anchor, positive, negative = y_pred[:int(Batch_size)], y_pred[int(Batch_size):int(2 * Batch_size)], y_pred[int(2 * Batch_size):]
        # anchor, positive, negative,2negative = y_pred[:int(Batch_size)], y_pred[int(Batch_size):int(2 * Batch_size)], y_pred[int(2 * Batch_size):int(3 * Batch_size)], y_pred[int(3 * Batch_size):]

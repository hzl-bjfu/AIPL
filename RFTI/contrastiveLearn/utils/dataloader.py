...
class FacenetDataset(Dataset):
    ...
    def load_dataset(self):
        for path in self.dataset_path:
            ...
            self.paths.append(path_split[1].split('\n')[0]) #cfy，根据我的路径，不要删掉空格
    ...
    def __getitem__(self, index):
        ...
        #   用来作为anchor和positive
        #------------------------------#
        c               = random.randint(0, self.num_classes - 1) #先随机选一个类别,randint函数可以选到0或num_classes - 1
        ...
        different_c_index   = np.random.choice(range(0, self.num_classes - 1), 1) #从0到num_classes-2中选一个索引（共num_classes-1个数）

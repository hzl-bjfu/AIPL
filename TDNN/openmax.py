...
def openmax(weibull_model, categories, input_score, eu_weight, alpha=10, distance_type='eucos'):
    ...
    for channel, input_score_channel in enumerate(input_score):
        ...
        for c, category_name in enumerate(categories):
            ...
            # print(wscore)
            # print('测试------------------------------------------------------------')
            # print(input_score_channel[c])
            modified_score = input_score_channel[c] * (1 - wscore * omega[c])
            my_modified_score = (1 - wscore * omega[c]) #自定义
            # if my_modified_score == 0:
            #     my_modified_score = 0.001
            # else:
            #     my_modified_score = 1/(1 - wscore * omega[c])   #自定义

            # my_score_channel.append(my_modified_score)   #自定义
        ...
        # my_scores.append(my_score_channel)  #自定义

    scores = np.asarray(scores)
    scores_u = np.asarray(scores_u)
    # my_scores = np.asarray(my_scores) #zidingyi
    # plda_prob = my_scores
    # print('myscore:',my_scores)
    # print('scores:',scores)
    # print('scores_u',scores_u)

    openmax_prob = np.array(compute_openmax_prob(scores, scores_u))
    # plda_prob = np.array(compute_openmax_prob(my_scores, 1-my_scores)) #自定义
    # print(plda_prob)
    # print('openmax_prob',openmax_prob)
...
def compute_train_score_and_mavs_and_dists(train_class_num,trainloader,device,net):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for score, t in zip(outputs, targets):  #tar里面没有这个类别
                # print('t:',t)

...
def InsFre_norm(x,epsilon=1e-5,batch=32,Fdim=128):
    if x.dim !=3:
        print('IFN got wrong input,expected 3D!')
    N = x.size(0)
    F = x.size(1)
    T = x.size(2)
    mean_list = []
    for b in range(N):
        for c in range(F):
            mean_list.append(torch.sum(x[b, c, :]) / T)

    var_list = []
    for b in range(N):
        for c in range(F):
            var_list.append(torch.var(x[b, c, :]))
